...
public class TerrainDetector : MonoBehaviour
{
    ...
    public int GetDominantTextureIndexAt(Vector3 worldPosition)
    {
        ...
        if (!ContainsIndex(alphamapCoordinates.x, dimension: 1))
            return -1;

        if (!ContainsIndex(alphamapCoordinates.z, dimension: 0))
            return -1;
        ...
        {
            Vector3 relativePosition = _worldPosition - ThisTerrain.transform.position;
    ...
    private bool ContainsIndex(int index, int dimension)
    {
        if (index < 0)
            return false;

        return index < CachedTerrainAlphamapData.GetLength(dimension);
    }
}
